...
foreach ($fields as $field) {
	...
	if (!empty($associations['belongsTo'])) {
		foreach ($associations['belongsTo'] as $alias => $details) {
			if ($field === $details['foreignKey']) {
				$isKey = true;
				echo "\t\t<dt class='col col-md-5 control-label'><?php echo __('" . Inflector::humanize(Inflector::underscore($alias)) . "'); ?></dt>\n";
	...
	if ($isKey !== true) {
		echo "\t\t<dt class='col col-md-5 control-label'><?php echo __('" . Inflector::humanize($field) . "'); ?></dt>\n";
