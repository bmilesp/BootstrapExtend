...
<div id="<?php echo $uuid; ?>">
	<input id='<?php echo $typeaheadFieldId ?>TagsInput' autofocus class='form-control twitter-typeahead' type="text" value="<?php echo $value ?>"  name='<?php echo $args['name']['name'] ?>' />
</div>
<script type="text/javascript">
	...
	$('#<?php echo $uuid; ?> .twitter-typeahead').typeahead(null, {
  		...
  		<?php if(!empty($options['typeahead']['templates'])){ ?>
  			templates : {
  				<?php if(!empty($options['typeahead']['templates']['empty'])){ ?>
  					empty : "<?php echo $options['typeahead']['templates']['empty']; ?>",
  				<?php } ?>	
  				<?php if(!empty($options['typeahead']['templates']['suggestion'])){ ?>
  					suggestion: Handlebars.compile("<?php echo $options['typeahead']['templates']['suggestion']; ?>")
  				<?php } ?>
  			}
  		<?php } ?>	
	})
